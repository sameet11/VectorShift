...
const Navbar=()=>{
    return(
        <div className='flex gap-5 pl-4 p-1 cursor-pointer'>
        ...
        {navElements.map((ele,id)=>{
            return(
                <div key={id} className={`flex items-center justify-center text-[#6C737F] text-sm font-semibold hover:text-[#9294F7] cursor-pointer ${id===0 ?'underlined':''}`}>{ele}</div>
            )
        })}
        </div>
    )
